...
internal static partial class GeneratorPipelines
{
    public static void ExecutePagesStorePipeline(SourceProductionContext context, Capture capture)
    {
        ...
        try
        {
            ...
            var pagePaths = Directory.GetFiles(pagesDir, "*.razor", SearchOption.AllDirectories);

            var typeMappings = new List<string>();
            foreach (var pagePath in pagePaths)
            {
                typeMappings.Add(DirectoryUtils.GetDirectoryToPageTypePair(pagePath, capture));
            }
